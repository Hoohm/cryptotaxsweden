...
class ReportWriter:
    ...
    def __rounding_report(self, threshold, report_filename):
    ...
    def __generate_k4_pages(self):
        ...
        fiat_events = [x for x in self.tax_events if Coin.is_fiat(x.name)]
        crypto_events = [x for x in self.tax_events if not Coin.is_fiat(x.name)]
    ...
    def __output_totals(self):
        crypto_tax_events = [x for x in self.tax_events if not Coin.is_fiat(x.name)]
        fiat_tax_events = [x for x in self.tax_events if Coin.is_fiat(x.name)]
    ...
    def create_coin_report(self, coin_report_filename, wallet):
        with open(coin_report_filename, "w") as f:
            ...
            coin_list = [coin for (_, coin) in wallet.items() if coin.amount > 1e-9]
    ...
    def create_k4_report(self, tax_events, stock_tax_events, personal_details):
        self.tax_events = tax_events
        self.stock_tax_events = stock_tax_events
        ...
        if self.cli_options.format == Format.sru and not self.cli_options.decimal_sru:
            if self.cli_options.rounding_report:
                ...
                self.__rounding_report(threshold, os.path.join(self.cli_options.out, "rounding_report.txt"))
